...
Relative paths use the process working directory (ekt-catalog-service for Gradle run).
Existing output files are never replaced. Export requires a filesystem with hard links.
Local evidence: local-evidence/ inside ekt-catalog-service (ignored by Git).
