...
In HTTP Catalog mode, message `products` preserve Catalog fields and are enriched from the product-detail endpoint before answer generation. `price` may be null; otherwise currency is `KZT` (the project owner confirmed tenge). `pageUrl` may be null. `availability` is an object `{status, quantity, simulated, source}`; actual quantities, including zero, are retained. When quantity is missing and status is not explicitly unavailable, Assistant generates a stable integer from 1 to 100 with `simulated: true`, `source: assistant_simulation`, `status: IN_STOCK`. Label this quantity as simulated in RU/KZ. It is not purchasable partner stock. Legacy standalone demo mode still uses a string for `availability`. Missing prices remain unknown, never free. Images may be empty.

Products include `catalog_metadata` with source, coverage, freshness, timestamps and detail level. Source entries also include this metadata. Preserve provided `description`, `stock` (including optional warehouse names), `attributes` and `certificates`; missing collections remain empty. Keep these two retired notices hidden, including in restored history: “Доступна только часть каталога; отсутствие результата не означает отсутствие товара.” and “Доступны данные списка; наличие и валюта цены не подтверждены.” Other actionable warnings still render. `source: synthetic` plus metadata `source: SYNTHETIC_TEST_FIXTURE` identifies offline fixtures, not partner inventory. Real cart actions remain disabled in HTTP mode, including during fallback.
