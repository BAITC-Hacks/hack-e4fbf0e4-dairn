    ...
    async def answer(self, evidence):
        ...
        response=await self.client.post('https://api.openai.com/v1/responses',headers={
            ...
            'instructions':('Ответь кратко по-русски. Используй только факты из evidence. '
                ...
                'Пустые attributes и certificates означают, что сведения не предоставлены, а не что характеристик или сертификатов не существует. '
                'Не добавляй единицы количества (шт, м и другие), если их нет в evidence. '
