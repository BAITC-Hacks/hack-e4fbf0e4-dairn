    ...
    async def answer(self, evidence):
        ...
        response=await self.client.post('https://api.openai.com/v1/responses',headers={
            ...
            'instructions':('Ответь кратко по-русски. Используй только факты из evidence. '
                ...
                'Цены ekt.kz указаны в KZT (тенге); используй переданную сумму, не выдумывай цену. '
                'availability.quantity содержит количество; значение 0 означает отсутствие товара. '
                'Если availability.simulated=true, это смоделированный остаток для прототипа: укажи количество с пометкой «демо». '
                'Не выдавай смоделированный остаток за подтверждённое наличие ekt.kz или по конкретному складу. '
                'availability UNKNOWN без количества означает неизвестное наличие. '
                ...
                'Не повторяй общие технические уведомления о частичном каталоге или неподтверждённой валюте списка. '
