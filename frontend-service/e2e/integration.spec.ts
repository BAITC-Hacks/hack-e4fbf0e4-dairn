...
test('HTTP catalog preserves unknown values, source metadata and API text without cart selection', async ({
  ...
  await expect(cards.nth(1)).toContainText('Валюта неизвестна');
  await expect(cards.nth(1)).not.toContainText('KZT');
  await cards
    .first()
  ...
  await expect(cards.nth(1)).toContainText('Валютасы белгісіз');
